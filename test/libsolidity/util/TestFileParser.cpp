...
tuple<bytes, ABIType, string> TestFileParser::parseABITypeLiteral()
{
	try
	{
		...
		{
			if (accept(Token::Hex, true))
			{
				string parsed = parseHexNumber();
				rawString += parsed;
				bytes converted = convertHexString(parsed);
				abiType = ABIType{ABIType::HexString, ABIType::AlignLeft, converted.size()};
				return make_tuple(converted, abiType, rawString);
			}
			else if (accept(Token::Boolean))
			{
